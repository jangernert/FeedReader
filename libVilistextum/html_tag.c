...
void html_tag(int nooutput, int spaces, int paragraph, int breite)
{
	...
	if CMP("SCRIPT", str)       { start_nooutput(nooutput, spaces, breite); }
	else if CMP("/SCRIPT", str) { end_nooutput(nooutput, spaces, breite); }
	else if CMP("STYLE", str)   { start_nooutput(nooutput, spaces, breite); }
	...
	if (nooutput==0) {
		...
		/* Linebreak */
		else if CMP("BR", str)  { line_break(nooutput, spaces, breite); }
		else if CMP("BR/", str) { line_break(nooutput, spaces, breite); } /* xhtml */

		else if CMP("P", str)  { start_p(nooutput, spaces, paragraph, breite); }
		else if CMP("/P", str) { paragraphen_ende(nooutput, spaces, paragraph, breite); }
		else if CMP("BLOCKQUOTE", str)  { start_p(nooutput, spaces, paragraph, breite); }
		...
		/* headings */
		else if CMP("H1", str)  { start_p(nooutput, spaces, paragraph, breite);          }
		else if CMP("/H1", str) { paragraphen_ende(nooutput, spaces, paragraph, breite); }
		else if CMP("H2", str)  { start_p(nooutput, spaces, paragraph, breite);          }
		else if CMP("/H2", str) { paragraphen_ende(nooutput, spaces, paragraph, breite); }
		else if CMP("H3", str)  { start_p(nooutput, spaces, paragraph, breite);          }
		else if CMP("/H3", str) { paragraphen_ende(nooutput, spaces, paragraph, breite); }
		else if CMP("H4", str)  { start_p(nooutput, spaces, paragraph, breite);          }
		else if CMP("/H4", str) { paragraphen_ende(nooutput, spaces, paragraph, breite); }
		else if CMP("H5", str)  { start_p(nooutput, spaces, paragraph, breite);          }
		else if CMP("/H5", str) { paragraphen_ende(nooutput, spaces, paragraph, breite); }
		else if CMP("H6", str)  { start_p(nooutput, spaces, paragraph, breite);          }
		else if CMP("/H6", str) { paragraphen_ende(nooutput, spaces, paragraph, breite); }

		else if CMP("HR", str)  { hr(nooutput, spaces, paragraph, breite); }
		else if CMP("HR/", str) { hr(nooutput, spaces, paragraph, breite); } /* xhtml */

		else if CMP("LI", str)    { start_lis(nooutput, spaces, breite); }
		else if CMP("/LI", str)   { end_lis(); }
		else if CMP("UL", str)    { start_uls(nooutput, spaces, breite); }
		else if CMP("/UL", str)   { end_uls(nooutput, spaces, breite); return; }
		else if CMP("DIR", str)   { start_uls(nooutput, spaces, breite); }       /* deprecated */
		else if CMP("/DIR", str)  { end_uls(nooutput, spaces, breite); return; } /* deprecated */
		else if CMP("MENU", str)  { start_uls(nooutput, spaces, breite); }       /* deprecated */
		else if CMP("/MENU", str) { end_uls(nooutput, spaces, breite); return; } /* deprecated */
		else if CMP("OL", str)    { start_ols(nooutput, spaces, breite); }
		else if CMP("/OL", str)   { end_ols(nooutput, spaces, breite); }

		else if CMP("DIV", str)      { start_div(0, nooutput, spaces, breite); }
		else if CMP("/DIV", str)     { end_div(nooutput, spaces, paragraph, breite); }
		else if CMP("CENTER", str)   { start_div(CENTER, nooutput, spaces, breite); } /* deprecated */
		...
		else if CMP("/TABLE", str)   { paragraphen_ende(nooutput, spaces, paragraph, breite); }
		else if CMP("TD", str)       { wort_plus_ch(' '); }
		else if CMP("/TD", str)      {}
		else if CMP("TH", str)       { wort_plus_ch(' '); }
		else if CMP("/TH", str)      {}
		else if CMP("TR", str)       { line_break(nooutput, spaces, breite); } /* start_p();  */
		else if CMP("/TR", str)      { /*paragraphen_ende();*/ }
		else if CMP("CAPTION", str)  {}
		else if CMP("/CAPTION", str) {}

		else if CMP("PRE", str)   { start_p(nooutput, spaces, paragraph, breite);  pre=1; }
		else if CMP("/PRE", str)  { paragraphen_ende(nooutput, spaces, paragraph, breite); pre=0; }

		else if CMP("DL", str)  { start_dl(nooutput, spaces, paragraph, breite);} /* Definition List */
		else if CMP("/DL", str) { end_dl(nooutput, spaces, paragraph, breite); }
		else if CMP("DT", str)  { start_dt(nooutput, spaces, breite); } /* Definition Title */
		else if CMP("/DT", str) { end_dt(); }
		else if CMP("DD", str)  { start_dd(nooutput, spaces, breite); } /* Definition Description */
