...
public class FeedReader.bazquxConnection {
	...
	{
		logger.print(LogMessage.DEBUG, "bazqux Connection: getToken()");
		...
		try{
			...
			{
				logger.print(LogMessage.ERROR, "Regex bazqux - %s".printf(response));
		...
		{
			logger.print(LogMessage.ERROR, "bazquxConnection - getToken: Could not load message response");
