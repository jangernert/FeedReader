...
public class FeedReader.WallabagAPI : ShareAccountInterface, Peas.ExtensionBase {
    ...
    public bool getAccessToken(string id, string username, string password)
    {
        ...
        string message = "grant_type=password"
		 				+ "&client_id=" + WallabagSecrets.clientID
						+ "&client_secret=" + WallabagSecrets.clientSecret
						+ "&username=" + username
						+ "&password=" + password;


		string url = WallabagSecrets.base_uri + "oauth/v2/token";
        ...
        var array = m_shareSettings.get_strv("wallabag");
        array += id;
		m_shareSettings.set_strv("wallabag", array);
    ...
    public bool addBookmark(string id, string url)
    {
		...
		{
			getAccessToken(id, getUsername(id), getPasswd(id));
		}
        ...
        var message_soup = new Soup.Message("POST", WallabagSecrets.base_uri + "api/entries.json");
        message_soup.set_request("application/x-www-form-urlencoded; charset=UTF8", Soup.MemoryUse.COPY, message.data);
    ...
    public bool logout(string id)
    {
		...
		m_shareSettings.set_strv("wallabag", array2);
    ...
    {
        return new WallabagSetup(id, username);
    }

    public ServiceSetup? newSetup()
    {
        return new WallabagSetup(null);
    }
}
