...
public class readerHeaderbar : Gtk.HeaderBar {
	...
	public readerHeaderbar () {
		...
		if(dataBase.read_propertie("only_unread") == 0)	{m_only_unread = false;}
		else											{m_only_unread = true; }

		if(dataBase.read_propertie("only_marked") == 0)	{m_only_marked = false;}
		else											{m_only_marked = true; }
